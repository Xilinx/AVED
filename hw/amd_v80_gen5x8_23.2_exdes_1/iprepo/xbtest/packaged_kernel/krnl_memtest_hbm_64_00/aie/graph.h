...
#if ((AIE_CONTROL == AIE_CONTROL_PL) && (AIE_STATUS == AIE_STATUS_PL))
class TopGraph : public graph
{
    public:
        ...
        TopGraph()
        {
            in0   = input_plio::create("in0",  plio_128_bits, "data/input0.txt",  PL_FREQ_MHZ);
            out0 = output_plio::create("out0", plio_128_bits, "data/output0.txt", PL_FREQ_MHZ);
            ...
            connect<stream> nout (krnl[NUMCORES-1].out[0], out0.in[0]);
            fifo_depth(nout) = FIFO_DEPTH;

            for (uint i=0; i<NUMCORES-1; i++)
            {
                connect<stream> n0 (krnl[i].out[0], krnl[i+1].in[0]);
...
class TopGraph : public graph
{
    public:
        ...
        TopGraph()
        {
            ...
            krnl[NUMCORES-1] = kernel::create(compute_end);
            runtime<ratio>(krnl[NUMCORES-1]) = 1.0;
            source(krnl[NUMCORES-1]) = "compute.cc";
