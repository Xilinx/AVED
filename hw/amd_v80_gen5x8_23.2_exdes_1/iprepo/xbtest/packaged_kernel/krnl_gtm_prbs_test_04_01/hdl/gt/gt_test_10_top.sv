...
for (index=0; index < 4; index=index+1)
  begin
    ...
    gt_channel_10 chan_i (
        ...
        .Rx_Data            (gtwiz_userdata_rx_out[index * 32 +: 32]   ),
        .Rx_Hdr_Val         (rxheadervalid_out[index * 2]              ),
        .Rx_Header          (rxheader_out[index * 6 +: 2]              ),
        .Rx_Slip            (rxgearboxslip_in[index]                   ),

        .Tx_Data            (gtwiz_userdata_tx_in[index * 32 +: 32]    ),
        .Tx_Hdr_Val         (txsequence_in[index * 7]                  ),
