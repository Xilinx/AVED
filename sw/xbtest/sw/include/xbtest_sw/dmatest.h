...
public:
    DMATest( Logging * log, XbtestSwConfig * xbtest_sw_config, ConsoleMgmtSw * console_mgmt, Timer * timer, DeviceInterface * device, const Dma_Parameters_t & test_parameters );
    ~DMATest();
...
private:
    ...
    using DMAMemTypeCfg_t = struct DMAMemTypeCfg_t {
        std::string                 name;
    ...
    const DMAMemTypeCfg_t RST_DMAMEMTYPECFG = {
        .name                           = "",
        .tag                            = {},
        .nom_total_size                 = 0,
        .total_size                     = 0,
    ...
    using DMATestcaseCfg_t = struct DMATestcaseCfg_t {
        std::map<uint, DMAMemTypeCfg_t> memory;
    };
    ...
    Dma_Parameters_t                            m_test_parameters;
    std::vector<DMA_Test_Sequence_Parameters_t> m_test_sequence;
    int                                         m_page_size = 0;
    DMATestcaseCfg_t                            m_TC_Cfg;
    ...
    bool    SetOtherTestSequenceItSettings  ( const DMA_Test_Sequence_Parameters_t & test_seq_param, const std::list<TestItConfig_t> & test_list, TestItConfig_t & test_it_cfg );
    void    SetTestSequenceStr              ( const DMA_Test_Sequence_Parameters_t & test_seq_param, TestItConfig_t & test_it_cfg );
    void    SetTestSequenceStrDbg           ( TestItConfig_t & test_it_cfg );
    void    AppendTestIt                    ( const DMA_Test_Sequence_Parameters_t & test_seq_param, TestItConfig_t & test_it_cfg, std::list<TestItConfig_t> & test_list, std::vector<std::string> & test_seq_strvect, std::vector<std::string> & test_seq_strvect_dbg, bool & parse_it_failure );
